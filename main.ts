 ...
 */
function createPizzaSizeOptionHTML(size: PizzaSize): string {
  return `
 ...
 */
export async function renderPizzas(): Promise<void> {
  ...
  let filterList:Pizza[] = structuredClone(cachedPizzas); 


const activeBtn = document.querySelector(".nav-menu .active") as HTMLButtonElement;
...
if (activeBtn && activeBtn.id !== "all") {
  ...
  filterList.forEach((pizza: Pizza) => {
    ...
    const smallerSizeButton = pizzaItemDiv.querySelector('.pizza-sizes > .pizza-size-option:nth-child(1) .add-pizza-button') as HTMLButtonElement;
    const biggerSizeButton = pizzaItemDiv.querySelector('.pizza-sizes > .pizza-size-option:nth-child(2) .add-pizza-button') as HTMLButtonElement;
...
function saveOrderListToLocalStorage(): void {
  try {
 ...
 */
function loadOrderListFromLocalStorage(): void {
  try {
    ...
    if (storedOrderList) {
      ...
      const parsedList: OrderItem[] = JSON.parse(storedOrderList);
...
async function addToOrder(pizzaName: string, isSmallerSize: boolean): Promise<void> {
  ...
  const existingOrderItem = orderList.find(
    item => item.pizzaName === pizza.name && item.isSmall === isSmallerSize
  );
...
function changeQuantity(pizzaName: string, isSmall: boolean, delta: number): void {
  const itemIndex = orderList.findIndex(
    item => item.pizzaName === pizzaName && item.isSmall === isSmall
  );
 ...
 */
function removeItem(pizzaName: string, isSmall: boolean): void {
  const itemIndex = orderList.findIndex(
    item => item.pizzaName === pizzaName && item.isSmall === isSmall
  );
...
async function renderOrderList(orderListSectionId: string = 'current-order-list'): Promise<void> {
    ...
    if (orderList.length === 0) {
        ...
        if(orderItemCount)orderItemCount.innerHTML=orderList.length.toFixed(0);
        const total =document.getElementById("total-order-price");
        if(total)total.innerHTML="0 грн";
  ...
  let totalNumber:number = 0;
    orderList.forEach((orderItem: OrderItem) => {
        ...
        const decrementButton = orderItemDiv.querySelector('.decrement') as HTMLButtonElement;
        const incrementButton = orderItemDiv.querySelector('.increment') as HTMLButtonElement;
        const removeButton = orderItemDiv.querySelector('.order-item-remove') as HTMLButtonElement;
  ...
  if(total)total.innerHTML=totalNumber.toFixed(2)+" грн";
...
if (typeButtons) {
    ...
    if(clearOrderButton)clearOrderButton.addEventListener('click', ()=>{
      orderList.length=0; renderOrderList();
    }); 
