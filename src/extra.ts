...
import { OrderItem, Pizza, PizzaSize } from './pizza-interfaces.js'; 
...
let orders: OrderItem[] = await getOrders(); // Ensure OrderItem is an array
...
const consolidatedPizzaData = new Map<string, any>();
...
const pivotData: any[] = Array.from(consolidatedPizzaData.values());
...
async function getOrder(): Promise<OrderItem> {
  try {
    ...
    const data: OrderItem= await response.json(); // Parse the JSON data
